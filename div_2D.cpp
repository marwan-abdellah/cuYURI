...
namespace div_2D
{
	namespace globalMem
	{
		char* appName = "div_2D_GM";

		void run(int argc, char** argv);
	}
}

void div_2D::globalMem::run(int argc, char** argv)
{
	float* inArr1_H;
	float* inArr2_H;
	float* outArr_H;
	...
	app::initApp(div_2D::globalMem::appName);
	...
	benchFile::header(div_2D::globalMem::appName); 
	...
	// Run Kernel 
	bench::cpu::startTimer(& _timer);
	CU::arr_2D::globalMem::cuDivArrays_f (inArr1_D, inArr2_D, outArr_D, __pitch, arrayWidth, arrayHeight, &conf);
	bench::cpu::stopTimer(& _timer); 
