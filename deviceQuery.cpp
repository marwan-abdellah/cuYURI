...
void deviceQuery::queryGPU( int argc, char** argv) 
{
	pArg = &argc;
	pArv = argv;
	...
	/* Create ".info" file */
    shrSetLogFileName ("GPU_.info");

    shrLog("CUDA Device Query, (Runtime API) version (CUDART static linking) \n");
        
	cudaError_t error_id = cudaGetDeviceCount(&deviceQuery::deviceCount);

    if (error_id != cudaSuccess) 
	{
        shrLog("cudaGetDeviceCount returned [%d] \n %s \n", (int)error_id, cudaGetErrorString(error_id));
        shrQAFinishExit(*pArg, (const char **)pArv, QA_FAILED)
    }

    /* This function call returns 0 if there are no CUDA capable devices */ 
    if (&deviceQuery::deviceCount == 0)
        shrLog("There is no device supporting CUDA \n");
    else
        shrLog("Found [%d] CUDA capable device(s) \n", deviceQuery::deviceCount);

          
    for (devItr = 0; devItr < deviceQuery::deviceCount; ++devItr) 
	{
       
		
		
		cudaGetDeviceProperties(&deviceProp, devItr);

        shrLog("\nDevice %d: \"%s\"\n", devItr, deviceProp.name);
	...
	// This is not available in the CUDA Runtime API, so we make the necessary calls the driver API to support this for output
        int memoryClock;
        getCudaAttribute<int>( &memoryClock, CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE, devItr );
        shrLog("  Memory Clock rate:                             %.2f Mhz\n", memoryClock * 1e-3f);
        int memBusWidth;
        getCudaAttribute<int>( &memBusWidth, CU_DEVICE_ATTRIBUTE_GLOBAL_MEMORY_BUS_WIDTH, devItr );
        shrLog("  Memory Bus Width:                              %d-bit\n", memBusWidth);
        int L2CacheSize;
        getCudaAttribute<int>( &L2CacheSize, CU_DEVICE_ATTRIBUTE_L2_CACHE_SIZE, devItr );
        if (L2CacheSize) {
    ...
    for (devItr = 0; devItr < ((deviceCount > 2) ? 2 : deviceCount); ++devItr) 
    {
