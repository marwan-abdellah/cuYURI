...
namespace sub_1D
{
	...
	void sub_1D::run_sub_1D(int argc, char** argv);
}
...
void sub_1D::run_sub_1D(int argc, char** argv)
{
	...
	benchFile::header(sub_1D::appName); 
	...
	benchFile::header("Benchmarking GPU Kernel"); 
	benchFile::bench(& _timer, "Upload time"); 

	// Run Kernel 
	bench::cpu::startTimer(& _timer);
	CU::arr_1D::cuSubArrays_f (inArr1_D, inArr2_D, outArr_D, arrayLenght, &conf);
	bench::cpu::stopTimer(& _timer); 
