...
namespace mul_2D
{
	namespace globalMem
	{
		char* appName = "mul_2D_GM";

		void run(int argc, char** argv);
	}
}

void mul_2D::globalMem::run(int argc, char** argv)
{
	float* inArr1_H;
	float* inArr2_H;
	float* outArr_H;
	...
	app::initApp(mul_2D::globalMem::appName);
	...
	benchFile::header(mul_2D::globalMem::appName); 
	...
	// Run Kernel 
	bench::cpu::startTimer(& _timer);
	CU::arr_2D::globalMem::cuMulArrays_f (inArr1_D, inArr2_D, outArr_D, __pitch, arrayWidth, arrayHeight, &conf);
	bench::cpu::stopTimer(& _timer); 
